...
const makeHtml = async (data, fs_path) => {
    try {
        // read styles
        var style = '';
...
exportHtml = async (data, filename) => {
...
const main = async () => {
    const md_text = await fs.readFile("./input.md", { encoding: "utf-8" });
    const content = convertMarkdownToHtml("./input.md", "pdf", md_text);
    const html = await makeHtml(content, "./");
    await exportPdf(html, "./output.pdf", "pdf", "./");
    read_pdf("./output.pdf")
    console.log("done");
}
main()
